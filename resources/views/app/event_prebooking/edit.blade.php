...
@section('contents')
    <div class="card">
        <div class="card-body">
            <h4 class="card-title">Event Pre-Booking Details Edit</h4>
            <hr>
            <form id="propertyRegistrationFrom" action="{{ route('event-pre-booking.update_details',$summary->id) }}" method="POST"   enctype="multipart/form-summary">
                @csrf
                @method('post')
                    <input type="hidden" name="event_pre_booking_id" value="{{ $summary->id }}">
